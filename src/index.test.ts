...
import {
  ListOptions,
  generateClientKeySpace,
  generate as generateNormal,
} from './index.js';

type Suite = {
  name: string;
  generate: typeof generateNormal;
  key: (clientID: string, name: string, id: string) => string;
};

const suites: Suite[] = [
  {
    name: 'normal',
    generate: generateNormal,
    key: (_: string, name: string, id: string) => `${name}/${id}`,
  },
  {
    name: 'client keys',
    generate: generateClientKeySpace,
    key: (clientID: string, name: string, id: string) =>
      `-/c/${clientID}/${name}/${id}`,
  },
];

for (const s of suites) {
  const {generate, name, key} = s;
  suite(name, () => {
    const e1 = z.object({
      id: z.string(),
      str: z.string(),
    ...
    suite('set', () => {
      ...
      for (const c of cases) {
        test(c.name, async () => {
          ...
          const actual = await rep.query(tx =>
            tx.get(key(tx.clientID, 'e1', id)),
          );
      ...
      for (const c of cases) {
        test(c.name, async () => {
          ...
          expect(actual).deep.eq(
            c.expectError ? undefined : c.expected,
            c.name,
          );
        });
      }
    ...
    suite('list', () => {
      ...
      for (const c of cases) {
        test(c.name, async () => {
          ...
          await rep.mutate.directWrite({
            key: key(rep.clientID, 'e1', 'foo'),
            val: {id: 'foo', str: 'foostr'},
          });
          await rep.mutate.directWrite({
            key: key(rep.clientID, 'e1', 'bar'),
            val: {id: 'bar', str: 'barstr'},
          });
          await rep.mutate.directWrite({
            key: key(rep.clientID, 'e1', 'baz'),
